    ...
    public function verify_token($token,$email)
    {
        try {
            if (isset($token)) {
                ...
                $stmt = $this->connect()->prepare("SELECT verify_token,verify_status FROM accounts WHERE verify_token = ? AND verification_expires > now()  LIMIT 1");
                ...
                if ($stmt->execute(array($token))) {
                    ...
                    if ($row["verify_status"] == 0) {
                        ...
                        $update_stmt = $this->connect()->prepare("UPDATE accounts SET verify_status=1,verification_expires=NULL WHERE verify_token=$clicked_token AND users_email=?");
